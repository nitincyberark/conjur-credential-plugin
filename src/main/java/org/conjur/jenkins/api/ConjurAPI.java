...
public class ConjurAPI {
	...
	public static String getAuthorizationToken(OkHttpClient client, ConjurConfiguration configuration,
		...
		if (context != null) {
			if (context instanceof Run) {
				...
				LOGGER.log(Level.FINE, ">>>>>>>>>>>>>>Check context: "+ (context));
				//LOGGER.log(Level.FINE, ">>>>>>>else>>>>>>>Check: "+ (context instanceof AbstractItem));
	...
	private static ConjurConfiguration inheritedConjurConfiguration(Item job) {
		LOGGER.log(Level.FINE, "251 job :"+job);
...
//		for (ItemGroup<? extends Item> g = 
//				job.getParent(); g instanceof AbstractFolder; g = ((AbstractFolder<? extends Item>) g).getParent()) {
